...
def _sincronizar_person_purchases(pg_conn, portal: dict) -> int:
    ...
    cur.execute("""
        SELECT DISTINCT documento, serie
    ...
    try:
        ...
        while True:
            ...
            psycopg2.extras.execute_batch(
                fc,
                "INSERT INTO person_purchases (store_id, bill, serie) VALUES (%s, %s, %s)"
                " ON CONFLICT (store_id, bill, serie) DO NOTHING",
                [(portal["store_id"], row[0], row[1]) for row in batch],
                page_size=500,
            )
            faciais_conn.commit()
            total += len(batch)
